...
Developed using Next.js and Bright Data's webunlocker, site is designed to assist users when shopping. Notifies users when a product drops in price and helps competitors by alerting them when the product is out of stock.
...
👉 The application features an interactive header with a carousel that highlights key features and benefits while incorporating a search bar where users can input Amazon product links for scraping. It includes a scraped products dashboard that displays detailed information about scraped products, such as images, titles, pricing, and other relevant details, with the ability to view and track individual product data.

👉 Users can opt-in for tracking specific products through a modal that allows them to provide their email addresses and receive notifications for important updates, such as back-in-stock alerts or lowest price notifications. To ensure the data remains current, the system leverages automated periodic scraping through cron jobs, keeping product information up-to-date and reliable.
...
Create a `.env` in the root of your project and add the following content:
...
Replace the placeholder values with your actual credentials, obtained by signing up on these specific websites from [BrightData](https://brightdata.com/), [MongoDB](https://www.mongodb.com/), and [Node Mailer](https://nodemailer.com/)
